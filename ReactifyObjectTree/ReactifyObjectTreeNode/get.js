...
async function getAsync() {
  try {
    ...
    let result = await this.beforeGet.call(bindThis)
    if (result !== undefined) value = result
    await this.event.emit("beforeGet", value).once 

    result = await this.afterGet.call(bindThis)
    if (result !== undefined) value = result
...
function getSync() {
  ...
  let result = this.beforeGet.call(bindThis)
  if (result !== undefined) value = result
  this.event.emit("beforeGet", value).once 

  result = this.afterGet.call(bindThis)
  if (result !== undefined) value = result
